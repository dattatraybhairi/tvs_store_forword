void find_uinque_pairs()
{
  ...
  for (uint8_t i = 0; i < a; i++)
  {
    ...
    if (unique_flag == 1)
    {
      ...
      unique_pair_data[unique_count*data_per_tag+5] = radioPktBuffer[14]; // RSSI
      unique_count++;
    }
  }
  ...
  {
    if (ID1_HB == health_data[j*6] && ID1_LB == health_data[j*6+1])
    unique_flag = 0;
  }
  if (unique_flag == 1)
  {
    ...
    health_data[health_count*data_per_tag + 5] = radioPktBuffer[14]; // RSSI
    health_count++;
  }
}
